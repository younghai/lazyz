...
//
// LazyZ port of the upstream Codex teammode state model. ZCode has no thread
// primitive: members are one-shot Agent-tool calls whose final message IS the
// deliverable. The upstream `threadId` / `threadTitle` / `bindThread` machinery
// is dropped; a member's identity is its `id` + `focus` + `name` only. A optional
// `runId` records which Agent invocation produced the member's last result.
//
...
const LOCK_TIMEOUT_MS = Number.parseInt(process.env.LAZYCODEX_TEAMMODE_LOCK_TIMEOUT_MS ?? process.env.OMO_TEAMMODE_LOCK_TIMEOUT_MS ?? "10000", 10);
const LOCK_RETRY_MS = Number.parseInt(process.env.LAZYCODEX_TEAMMODE_LOCK_RETRY_MS ?? process.env.OMO_TEAMMODE_LOCK_RETRY_MS ?? "25", 10);
...
function assertUniqueMemberName(team) {
	...
	for (const member of team.members) {
		...
		const previous = seen.get(key);
		if (previous) {
			throw new Error(`member name "${memberName}" duplicates "${previous.name ?? previous.focus}" (no two members may share a name)`);
		}
		seen.set(key, member);
	}
}
...
function assertTeamReady(team) {
	if (isUnderstaffed(team)) {
		throw new Error(
			`team needs at least ${MIN_MEMBERS} distinct members before dispatch; current count is ${team.members.length}`,
		);
	}
...
export function addMember(team, { id, focus, lens, deliverable = "", branch = null, name = null }) {
	...
	// The member name is the short role label. Fall back to the focus so every member is
	// distinguishable.
	...
	if (duplicateName) {
		throw new Error(`member name "${memberName}" duplicates "${duplicateName.name ?? duplicateName.focus}" (no two members may share a name)`);
	}
	team.members.push({
		id: memberId,
		name: memberName,
		...
		// runId: optional handle the leader sets to track which Agent-tool call this
		// member's last result came from. null until the leader dispatches the member.
		runId: null,
		cwd: null,
...
export function setMemberRunId(team, { id, runId }) {
	const m = memberById(team, id);
	m.runId = typeof runId === "string" && runId.trim() ? runId.trim() : null;
	return touch(team, "set-run-id", `member ${id} -> run ${m.runId ?? "(cleared)"}`);
}
...
export function validateTeam(team) {
	if (team?.schemaVersion !== 3) throw new Error("invalid team: schemaVersion must be 3 (LazyZ teammode port)");
