...
// (1) vendored source — bundled into the plugin under vendor/lsp-tools-mcp
// (2) 5-up repo sibling — has package.json, can be built (repo checkout layout)
// (3) 2-up components sibling — bundled dist-only inside the installed plugin cache
const candidates = [
	join(__dirname, "..", "..", "..", "vendor", "lsp-tools-mcp"),
...
for (const dir of candidates) {
	...
	if (existsSync(packageJson)) {
		...
		execSync("npm install --no-save --ignore-scripts", { cwd: dir, stdio: "inherit" });
