...
- 2 failed rounds on one checkbox → load the `debugging` skill, run its hypothesis-driven loop (≥3 hypotheses, validate cheapest first), and re-dispatch with the validated root cause in the failure context. Do not re-dispatch the same failing executor context blindly.
- 3 same-failure type on one checkbox → escalate via `multi_agent_v1.spawn_agent({"message":"TASK: act as a rigorous reviewer. DELIVERABLE: diagnose the repeated sub-task failure and recommend the next safe action. VERIFY: cite the failing evidence.","fork_context":false})` and stop dispatch. Increment boulder.json `fail_count`.
- 5 execution cycles on one checkbox without a `confirmed` verdict → set boulder work `status` to `"blocked"`, increment `fail_count`, append a `checkbox-blocked` ledger entry with the diagnosis, and stop dispatch. The continuation hook will not resume a blocked work.
- `AdversarialVerify.verdict === "needs-human-review"` → set boulder work `status` to `"blocked"`, increment `fail_count`, record the question in the ledger, and surface it to the user. Do not re-dispatch; this is an explicit escalation.
