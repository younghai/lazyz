...
for (const workspace of workspaces) {
	...
	// LazyZ: build with bun (npm not assumed). bun --filter matches by package
	// name, so read the workspace's package.json name.
	const wsPkg = JSON.parse(await readFile(join(root, workspace, "package.json"), "utf8"));
	run("bun", ["run", "--filter", wsPkg.name, "build"], root);
...
for (const componentName of await readStandaloneComponentNames()) {
	...
	run("bun", ["run", "build"], join(root, componentPath));
